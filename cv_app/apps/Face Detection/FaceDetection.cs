...
namespace OpenCVForUnityExample
{
    ...
    public class FaceDetection : MonoBehaviour
    {
        ...
        string pathCascade = "lbpcascade_frontalface.xml";
        Mat imgGray;
        MatOfRect faces;
        ...
        void Start()
        {
            ...
            cascade.load(Utils.getFilePath(pathCascade));
            if (cascade.empty())
            {
        ...
        void Update()
        {
            if (webCamTextureToMatHelper.IsPlaying() && webCamTextureToMatHelper.DidUpdateThisFrame())
            {
                ...
                if (cascade != null)
                {
                    cascade.detectMultiScale(imgGray, faces, 1.1, 5);
                }

                OpenCVForUnity.CoreModule.Rect[] bboxs = faces.toArray();

                for (int i = 0; i <bboxs.Length; i++)
                {
                    Imgproc.rectangle(img, bboxs[i].tl(), bboxs[i].br(), new Scalar(255, 0, 0, 255), 2);
